...
*/
  ...
  if( $TBDEV['captcha'] )
  {
    ...
    {
        header("Location: {$TBDEV['baseurl']}/members.php?action=reg");
        exit();
    }
  }
  ...
  */
  ...
  $body = str_replace(array('<#SITENAME#>', '<#USEREMAIL#>', '<#IP_ADDRESS#>', '<#REG_LINK#>'),
                      array($TBDEV['site_name'], $email, $_SERVER['REMOTE_ADDR'], "{$TBDEV['baseurl']}/members.php?action=confirm&id=$id&secret=$psecret"),
