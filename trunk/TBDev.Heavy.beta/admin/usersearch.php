...
require_once "include/pager_functions.php";
    ...
    function is_set_not_empty($param) {
      if(isset($_POST[$param]) && (!empty($_POST[$param]) OR $_POST[$param] != '') )
         return true;
      else
         return false;
    }
    ...
    <form method='post' action='admin.php?action=usersearch'>
         <table border='1' cellspacing='0' cellpadding='5'>
               <tr>
                  ...
                  <td " .(is_set_not_empty('n')?$highlight:''). "><input name='n' type='text' value='" .(isset($_POST['n'])?htmlsafechars($_POST['n']):''). "' size='25' /></td>
                  <td valign='middle' class='rowhead'>{$lang['usersearch_ratio']}</td>
                  <td " .(is_set_not_empty('r')&&$_POST['r']>0?$highlight:''). ">
                     <select name='rt'>";

    $options = array("equal","above","below","between");
	for ($i = 0; $i < count($options); $i++){
	   $HTMLOUT .= "<option value='$i' ".(((isset($_POST['rt'])&&$_POST['rt']>0?$_POST['rt']:"3")=="$i")?"selected='selected'":"").">{$options[$i]}</option>\n";
	}

    $HTMLOUT .= "
                     </select>
                     <input name='r' type='text' value='" .(isset($_POST['r'])&&$_POST['r']>0? $_POST['r']:'')."' size='5' maxlength='4' />
                     <input name='r2' type='text' value='" .(isset($_POST['r2'])&&$_POST['r2']>0?$_POST['r2']:'')."' size='5' maxlength='4' />
                  </td>
                  <td valign='middle' class='rowhead'>{$lang['usersearch_status']}</td>
                  <td ".(is_set_not_empty('st')&&$_POST['st']>0?$highlight:'').">
                     <select name='st'>";
    ...
    $HTMLOUT .= "
               ...
               <tr>
                  ...
                  <td " .(is_set_not_empty('as')&&$_POST['as']>0?$highlight:''). "><select name='as'>";
...
/* - GAVE UP TO HERE COULDNT BE ARSED NAH WHAT I MEAN - */
    ...
    for ($i = 0; $i < count($options); $i++){
      $HTMLOUT .= "<option value='$i' ".(((isset($_POST['as'])&&$_POST['as']>0?$_POST['as']:"0")=="$i")?"selected='selected'":"").">".$options[$i]."</option>\n";
    }

    $HTMLOUT .="</select></td></tr>
<tr>
  <td valign='middle' class='rowhead'>{$lang['usersearch_comments']}</td>
  <td ".(is_set_not_empty('co')?$highlight:'').">
  <input name='co' type='text' value='".(is_set_not_empty('co')?$_POST['co']:'')."' size='25' /></td>
  <td valign='middle' class='rowhead'>{$lang['usersearch_mask']}</td>
  <td ".(is_set_not_empty('ma')?$highlight:'').">
  <input name='ma' type='text' value='".(is_set_not_empty('ma')?$_POST['ma']:'')."' maxlength='17' /></td>
  <td valign='middle' class='rowhead'>{$lang['usersearch_class']}</td>
  <td ".(is_set_not_empty('c')?$highlight:'')."><select name='c'><option value=''>(any)</option>";
  ...
  for ($i = 2;;++$i) 
  {
		if ($c = get_user_class_name($i-2))
       	 $HTMLOUT .= "<option value='" . $i . "'".((isset($class)?$class:0) == $i? " selected='selected'" : "") . ">$c</option>\n";
	  else
	   	break;
	}
...
<tr>
    ...
    <td valign='middle' class='rowhead'>{$lang['usersearch_joined']}</td>

  <td ".(is_set_not_empty('d')?$highlight:'').">
  <select name='dt'>";

	$options = array('on','before','after','between');
	for ($i = 0; $i < count($options); $i++)
	{
	  $HTMLOUT .= "<option value='$i' ".(((isset($_POST['dt'])?$_POST['dt']:'0')=='$i')?"selected='selected'":'').">{$options[$i]}</option>\n";
	}

    $HTMLOUT .= "</select>
    <input name='d' type='text' value='".(isset($_POST['d'])?$_POST['d']:'')."' size='12' maxlength='10' />

    <input name='d2' type='text' value='".(isset($_POST['d2'])?$_POST['d2']:'')."' size='12' maxlength='10' /></td>


  <td valign='middle' class='rowhead'>{$lang['usersearch_uploaded']}</td>

  <td ".(is_set_not_empty('ult')&&$_POST['ult']>0?$highlight:'')."><select name='ult' id='ult'>";

    $options = array("equal","above","below","between");
    for ($i = 0; $i < count($options); $i++){
  	  $HTMLOUT .= "<option value='$i' ".(((isset($_POST['ult'])&&$_POST['ult']>0?$_POST['ult']:"0")=="$i")?"selected='selected'":"").">{$options[$i]}</option>\n";
    }

    $HTMLOUT .= "</select>

    <input name='ul' type='text' id='ul' size='8' maxlength='7' value='".(isset($_POST['ul'])&&$_POST['ul']>0?$_POST['ul']:'')."' />

    <input name='ul2' type='text' id='ul2' size='8' maxlength='7' value='".(isset($_POST['ul2'])&&$_POST['ul2']>0?$_POST['ul2']:'')."' /></td>
  <td valign='middle' class='rowhead'>{$lang['usersearch_donor']}</td>

  <td ".(is_set_not_empty('do')&&$_POST['do']>0?$highlight:'')."><select name='do'>";

    $options = array("(any)","Yes","No");
	for ($i = 0; $i < count($options); $i++)
	{
	  $HTMLOUT .= "<option value='$i' ".(((isset($_POST['do'])&&$_POST['do']>0?$_POST['do']:"0")=="$i")?"selected='selected'":"").">{$options[$i]}</option>\n";
  }

	$HTMLOUT .= "</select></td></tr>
  <tr>

  <td valign='middle' class='rowhead'>{$lang['usersearch_lastseen']}</td>

  <td ".(is_set_not_empty('ls')?$highlight:"").">
  <select name='lst'>";

  $options = array("on","before","after","between");
  for ($i = 0; $i < count($options); $i++)
  {
    $HTMLOUT .= "<option value='$i' ".(((isset($_POST['lst'])?$_POST['lst']:"0")=="$i")?"selected='selected'":"").">{$options[$i]}</option>\n";
  }

  $HTMLOUT .= "</select>

  <input name='ls' type='text' value='".(isset($_POST['ls'])?$_POST['ls']:'')."' size='12' maxlength='10' />

  <input name='ls2' type='text' value='".(isset($_POST['ls2'])?$_POST['ls2']:'')."' size='12' maxlength='10' /></td>
	  <td valign='middle' class='rowhead'>{$lang['usersearch_downloaded']}</td>

  <td ".(is_set_not_empty('dl')?$highlight:'')."><select name='dlt' id='dlt'>";

	$options = array("equal","above","below","between");
	for ($i = 0; $i < count($options); $i++){
	  $HTMLOUT .= "<option value='$i' ".(((isset($_POST['dlt'])?$_POST['dlt']:"0")=="$i")?"selected='selected'":"").">".$options[$i]."</option>\n";
	}

    $HTMLOUT .= "</select>

    <input name='dl' type='text' id='dl' size='8' maxlength='7' value='".(isset($_POST['dl'])?$_POST['dl']:'')."' />

    <input name='dl2' type='text' id='dl2' size='8' maxlength='7' value='".(isset($_POST['dl2'])?$_POST['dl2']:'')."' /></td>

	<td valign='middle' class='rowhead'>{$lang['usersearch_warned']}</td>

	<td ".(is_set_not_empty('w')&&$_POST['w']>0?$highlight:'')."><select name='w'>";

  $options = array("(any)","Yes","No");
	for ($i = 0; $i < count($options); $i++){
		$HTMLOUT .= "<option value='$i' ".(((isset($_POST['w'])&&$_POST['w']>0?$_POST['w']:"0")=="$i")?"selected='selected'":"").">{$options[$i]}</option>\n";
  }
  ...
  <td valign='middle' class='rowhead'>{$lang['usersearch_active']}</td>
	<td ".(is_set_not_empty('ac')?$highlight:'')."><input name='ac' type='checkbox' value='1' ".(isset($_POST['ac'])?"checked='checked'":'')." /></td>
  <td valign='middle' class='rowhead'>{$lang['usersearch_banned']}</td>
  <td ".(is_set_not_empty('dip')?$highlight:'').">
  <input name='dip' type='checkbox' value='1' ".(isset($_POST['dip'])?"checked='checked'":'')." /></td>
  </tr>
  <tr>
  <td colspan='6' align='center'>
  <input name='submit' type='submit' class='btn' /></td>
  </tr>
</table>
<br /><br />
</form>";
...
if (count($_POST) > 0 );//&& isset($_POST['n']))
{
  ...
  if (isset($_POST['ip']) AND $_POST['ip'] !=='')
  {
    ...
    {
    	stderr($lang['usersearch_error'], $lang['usersearch_badip']);
    }
  ...
  if (is_set_not_empty('d'))
  {
  	...
  	{
    	stderr($lang['usersearch_error'], $lang['usersearch_baddate']);
      die();
    }
  ...
  if (is_set_not_empty('st') AND $_POST['st'] > 0)
  {
  	$status = $_POST['st'];
  	$where_is .= ((!empty($where_is))?" AND ":"");
    if ($status == "1")
  ...
  if (is_set_not_empty('as') AND $_POST['as'] > 0)
  {
  ...
  if (is_set_not_empty('do') AND $_POST['do'] > 0)
  {
		$donor = $_POST['do'];
		$where_is .= (!empty($where_is))?" AND ":"";
    if ($donor == 1)
  ...
  if (is_set_not_empty('w') AND $_POST['w'] > 0)
  {
		$warned = $_POST['w'];
		$where_is .= (!empty($where_is))?" AND ":"";
    if ($warned == 1)
  ...
  if (isset($_POST['ac']) AND $_POST['ac']== 1)
  {
  	$active = isset($_POST['ac']) ? $_POST['ac'] : '';
  	$distinct = "DISTINCT ";
...
//    <temporary>    /////////////////////////////////////////////////////
  if ($DEBUG_MODE > 0)
  {
  	//stdhead($lang['usersearch_window_title']);
  	$HTMLOUT .= "{$lang['usersearch_count']},$queryc
    <br /><br />
    {$lang['usersearch_query']},$query
    <br /><br />
    {$lang['usersearch_url']},$q";
  }
//    </temporary>   /////////////////////////////////////////////////////
//print_r($_POST);exit;
...
//    $auxres = mysql_query("SELECT COUNT(id) FROM posts WHERE userid = ".$user['id']) or sqlerr(__FILE__, __LINE__);
    ...
    <form method='post' action='sendmessage.php'>
      <table border='1' cellpadding='5' cellspacing='0'>
        <tr>
          <td>
            <div align='center'>
              <!--<input name='pmees' type='hidden' value='{$querypm}' size=10>-->
              <input name='pmees' type='hidden' value='".htmlsafechars(rtrim($ids, ':'))."' />
              <input name='PM' type='submit' value='PM' class='btn' />
              <input name='n_pms' type='hidden' value='".htmlsafechars($count)."' size='10' />
            </div></td>
        </tr>
      </table>
    </form>";
  ...
  print stdhead($lang['usersearch_window_title']) . $HTMLOUT . stdfoot();
  die;
