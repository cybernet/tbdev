...
stdhead("Staff Settings");
    ...
    begin_main_frame();
	begin_frame();
	print ("<form method='post' action='".$_SERVER["SCRIPT_NAME"]."'><input type='hidden' name='action' value='maxcoder'>");

	echo '<tr><td colspan="2" class="colhead" align="center">Add all site staff usernames and id\'s here</td></tr><br /><tr><td colspan="2" align="center"><b>Attention :</b> To view changes press refresh once you have saved a new entry</td></tr><tr><td colspan="2"align="center"><a href="maxcoder.php" target="mcoder"><b>Refresh</b></a></td></tr>';
    if (get_user_class() == UC_CODER) { 
	tr("Coder Names?","<textarea name=\"staffnames\" cols=\"70\" rows=\"4\" id=\"box\">".safeChar($staffnames)."</textarea><br /><font color=red>Enter only your highest class usernames here - Default is coder <br />- If you add classes remember to adjust the maxcoder settings !<b></font><font color=green><br /><u>Separate each staff member name with a space.</font></b></u>\n", 1);
    }
	if (get_user_class() >= UC_SYSOP) {
	tr("Staff Id's?","<textarea name=\"staffids\" cols=\"70\" rows=\"4\" id=\"box\">".safeChar($staffids)."</textarea><br /><font color=red>Enter all your staff ids here</font><b><u><br /><font color=green>Separate each staff member id's with a space.</font></b></u>\n", 1);
	}
