...
stderr( _("Warning"), sprintf( _("Unable to unlink file: %s. You should contact an administrator about this error."), "<b>".safeChar($a['filename'])."</b>") );
}
}
}
}
...
($mod || $arr["uppedby"] == safeChar($CURUSER["id"]) ? " <font size=1 class=small><a href=?delete=$arr[id]>[Delete]</a></font>" : "") ."</td>\n";
$added = "<td>" . substr($arr["added"], 0, 10) . "</td><td>" . substr($arr["added"], 10) . "</td>\n";
$size = "<td>" . safeChar(mksize($arr['size'])) . "</td>\n";
$hits = "<td>" . safeChar(number_format($arr['hits'])) . "</td>\n";
...
print("<tr><td class=rowhead>".('File')."</td><td align=left><input type=file name=file size=60><br>(".('Maximum file size').": ".safeChar($maxfilesize).")</td></tr>\n");
