...
echo "<b><font color=red size=3><b>Looks like its already done,youre to slow!<br /><a href=snatchleave.php>back ti the List</a></b></font>";
stdfoot();
die();
}
else{
mysql_query("UPDATE snatched SET sl_warned='yes' WHERE torrent='".$tid."' AND userid='".$uid."'")or sqlerr(__FILE__,__LINE__);
echo "<b><font color=red size=3>Succesful removed from the List</b></font>";
}
}
elseif ($_GET["remsl"] == "1")
{
...
if (get_user_class() < UC_MODERATOR)
stderr( _("Error"), _("Permission denied."));
$slr = mysql_query("SELECT sl_warned FROM snatched WHERE torrent='".$tid."' AND userid='".$uid."'")or sqlerr(__FILE__,__LINE__);
$sla = mysql_fetch_assoc($slr);
if ($sla["sl_warned"] == "no"){
...
mysql_query("UPDATE snatched SET sl_warned='no' WHERE torrent='".$tid."' AND userid='".$uid."'")or sqlerr(__FILE__,__LINE__);
echo "<b><font color=red size=3>Successfull added back to the list</b></font>";
}
}
//$scp = 172800;
$scp = 86400;
