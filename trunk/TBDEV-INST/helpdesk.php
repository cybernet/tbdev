...
mysql_query("DELETE FROM helpdesk WHERE id IN (" . implode(", ", $_POST[delhd]) . ")");
...
mysql_query("DELETE FROM helpdesk WHERE solved='yes' OR solved='ignored'") or sqlerr(__FILE__, __LINE__);
$action = 'problems';
}
...
$problem_count = mysql_query("select count(id) as problems from helpdesk WHERE solved = 'no'");
...
if (is_valid_id($id)) {
$res = mysql_query("SELECT * FROM helpdesk WHERE id =".$id) or sqlerr(__FILE__, __LINE__);
$arr = mysql_fetch_array($res);

$biatch_rez = mysql_query("SELECT username FROM users WHERE id =".$arr['added_by']) or sqlerr(__FILE__, __LINE__);
$biatch_arr = mysql_fetch_assoc($biatch_rez);

$solved_by_rez = mysql_query("SELECT username FROM users WHERE id =".$arr['solved_by']) or sqlerr(__FILE__, __LINE__);
$solved_by_arr = mysql_fetch_assoc($solved_by_rez);

echo'<table align=center border=1 cellpadding=5 cellspacing=0>'.
'<tr><td align=center colspan=2 class=colhead>'.htmlentities($arr['title']).'</td></tr>'.
'<tr><td align=right class=clearalt6><b>Added</b></td><td align=left class=clearalt6>On&nbsp;<b>'.$arr['added'].'</b>'.
'&nbsp;by&nbsp;<a class=altlink href=userdetails.php?id='.$arr['added_by'].'><b>'.($biatch_arr['username'] == '' ? 'Deleted member' : $biatch_arr['username']).'</b></a></td></tr>';

if ($arr['solved'] === 'yes')
echo'<tr><td align=right class=clearalt6><b>Problem</b></td><td align=left class=clearalt6><textarea name=msg_problem cols=80 rows=15 READONLY style="background-color:#332831; color:#f5f0c1;">'.$arr['msg_problem'].'</textarea></td></tr>'.
'<tr><td align=right class=colhead><b>Solved</b></td><td align=left class=colhead><font color=green><b>Yes</b></font>&nbsp;on&nbsp;<b>'.$arr['solved_date'].'</b>&nbsp;by&nbsp;<a class=altlink href=userdetails.php?id='.$arr['solved_by'].'><b>'.($solved_by_arr['username'] == '' ? 'Deleted Staff' : $solved_by_arr['username']).'</b></a></td></tr>'.
'<tr><td align=right class=clearalt7><b>Answer</b></td><td align=left class=clearalt7><textarea name=msg_answer cols=80 rows=15 READONLY style="background-color:#241a28; color:#f5f0c1;">'.$arr['msg_answer'].'</textarea></td></tr></table>';

if ($arr['solved'] === 'ignored')
echo'<tr><td align=right class=clearalt6><b>Problem</b></td><td align=left class=clearalt6><textarea name=msg_problem cols=80 rows=15>'.$arr['msg_problem'].'</textarea></td></tr>'.
'<tr><td align=right class=colhead><b>Solved</b></td><td align=left class=colhead><font color=orange><b>Ignored</b></font>&nbsp;on&nbsp;<b>'.$arr['solved_date'].'</b>&nbsp;by&nbsp;<a class=altlink href=userdetails.php?id='.$arr['solved_by'].'><b>'.($solved_by_arr['username'] == '' ? 'Deleted Staff' : $solved_by_arr['username']).'</b></a></td></tr>'.
'</table>';
...
$hd_reply['5'] = array('Die n00b','Die n00b! Such a thing knows even my grandma!');

$hd_answer = $_POST['hd_answer'];
$body = ($_POST['hd_answer'] !== '' ? $hd_reply[$hd_answer][1] : '');
$addedbyid = 0 + $arr['added_by'];

echo'<form method=post name=compose action=helpdesk.php><tr><td align=right class=clearalt6><b>Problem</b></td><td align=left class=clearalt6><textarea name=msg_problem cols=80 rows=15 READONLY style="background-color:#332831; color:#f5f0c1;">'.$arr['msg_problem'].'</textarea></td></tr>'.
'<tr><td align=right class=colhead><b>Solved</b></td><td align=center class=colhead align=left><font color=red><b>No</b></font><tr><td align=center class=clearalt7><b>Answer:</b></td><td align=center class=clearalt7>';
textbbcode('compose','body',$body);
echo'<input type=hidden name=id value='.$id.'><input type=hidden name=addedbyid value='.$addedbyid.'></td></tr>'.
'<tr><td colspan=2 align=center class=clearalt7> <script language="javascript" src="spellmessage.js"></script> '.
'<input type=button class=button value="Spell Check" onclick="return openspell(1);"> <input type=submit value="Answer question" class=button> '.
' <a class=altlink2 href=helpdesk.php?action=solve&pid='.$id.'&solved=ignored><input type=submit value="Ignore question" class=button></a></form></td></tr>'.
'<tr><td align=center colspan=2 class=colhead><form method=post action=helpdesk.php?action=problems&id='.$id.'><b>General Help Desk Replies:</b> <select name=hd_answer>';
...
$res_count = mysql_query("SELECT id FROM helpdesk") or sqlerr(__FILE__, __LINE__);
...
'<form method=post name=helpdesk action=?staff_delete=1 onSubmit="return ValidateForm(this,\'delhd\')">';

$res = mysql_query("SELECT added, solved_date, added_by, id, title, solved, solved_by FROM helpdesk ORDER BY added DESC $limit") or sqlerr(__FILE__, __LINE__);
...
mysql_query("UPDATE helpdesk SET solved='ignored', solved_by=".sqlesc($CURUSER['id']).", solved_date =".sqlesc(get_date_time())." WHERE id=".$pid) or sqlerr(__FILE__, __LINE__);
...
$added_by_rez = mysql_query("SELECT username FROM users WHERE id =".$addedbyid) or sqlerr(__FILE__, __LINE__);
...
mysql_query("UPDATE helpdesk SET solved='yes', solved_by=".sqlesc($CURUSER['id']).", solved_date = ".sqlesc(get_date_time()).", msg_answer = ".sqlesc($msg_answer)." WHERE id=".$id) or sqlerr(__FILE__, __LINE__);

mysql_query("INSERT INTO messages (sender, subject, receiver, added, msg, poster,unread) VALUES(".sqlesc($CURUSER['id']).", 'Help Desk Question Answered', ".sqlesc($addedbyid).", ".sqlesc(get_date_time()).", $msg, ".sqlesc($CURUSER['id']).",'yes')");
...
mysql_query("INSERT INTO helpdesk (title, msg_problem, added, added_by) VALUES (".sqlesc($title).", ".sqlesc($msg_problem).", ".sqlesc(get_date_time()).", ".sqlesc($CURUSER['id']).")") or sqlerr(__FILE__, __LINE__);
...
echo'<table border=0 align=center cellspacing=0 width=95%>'.
'<tr><td align=center class=colhead colspan=2><h1>Help Desk</h1></td></tr><tr><td align=center class=clearalt6 colspan=2>'.
'Before using <b>Help Desk</b> make sure to read the <a class=altlink href=faq.php><b>FAQ</b></a> '.
'and search <a class=altlink href=forums.php><b>Forums</b></a> first.<br>All questions that are answered there will be Ignored.<br/></td></tr>'.
'<tr><td class=clearalt6 align=right><form method=post name=compose action=helpdesk.php?action=help><b>Title:</b></td><td class=clearalt6><input type=text size=73 maxlength=60 name=title></td></tr>'.
'<tr><td class=clearalt6 align=right><b>Question: </b></td><td class=clearalt6><textarea name=msg_problem cols=80 rows=5></textarea></td></tr>'.
'<tr><td align=center class=clearalt6 colspan=2><input type=submit value="Help me!" class=button></td></tr></table></form>';
echo '</table>';
stdfoot();
?>
