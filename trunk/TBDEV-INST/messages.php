...
global $CURUSER;
$res = mysql_query('SELECT boxnumber,name FROM pmboxes WHERE userid='.$CURUSER['id']. ' ORDER BY boxnumber') or sqlerr(__FILE__,__LINE__);
...
mysql_query("UPDATE users SET pms_per_page = ".sqlesc($num_of_pms_per_page_ssti)." WHERE id = ".$CURUSER['id']) or sqlerr(__FILE__,__LINE__);
...
mysql_query("UPDATE users SET show_pm_avatar = ".sqlesc($show_pm_avatar)." WHERE id = ".$CURUSER['id']) or sqlerr(__FILE__,__LINE__);
if ($_GET['edit_mail_boxes'])
...
$res_box_name = mysql_query('SELECT name FROM pmboxes WHERE userid='.sqlesc($CURUSER['id']).' AND boxnumber='.sqlesc($mailbox).' LIMIT 1') or sqlerr(__FILE__,__LINE__);
...
$res_count = mysql_query("SELECT COUNT(*) FROM messages WHERE ".($mailbox === PM_INBOX ? "receiver=".$CURUSER['id']." AND location = 1" : ($mailbox === PM_SENTBOX ? "sender=".$CURUSER['id']." AND (saved = 'yes' || unread= 'yes')" : "receiver=".sqlesc($CURUSER['id'])." AND location=".sqlesc($mailbox)))) or sqlerr(__FILE__,__LINE__);
...
echo '<table width=95%><tr><td colspan=5 class=colhead align=center>'.$other_box_note.'<font size=1>'.$messages.' / '.maxbox($CURUSER['class']).'</font>&nbsp;&nbsp;&nbsp;&nbsp;<font size=4>'.$mailbox_name.'</font>'.
...
if ($row['user_id'] !== 0){
$res_friend = mysql_query("SELECT * FROM friends WHERE userid=".$CURUSER['id']." AND friendid=".sqlesc($row['user_id'])) or sqlerr(__FILE__, __LINE__);
...
'<br><div align=center><table><tr><td>'.$per_page_drop_down.'</td><td>'.$show_pm_avatar_drop_down.'</td></tr></table><br>'.
'<a class=altlink href=?action=search>Search Messages</a> || <a class=altlink href=?action=editmailboxes>Mailbox Manager / PM settings</a> || <a class=altlink href=/sendmessage.php?receiver='.$CURUSER['id'].'&draft=1>write new Draft</a></div>';
stdfoot();
}
...
//=== print the damn thing :P
echo'<tr><td class='.$class.'><img src=pic/outbox.gif alt=Read></td><td class='.$class.'>'.
'<a class=altlink href=?action=viewmessage&id='.$row['id'].'>'.($row['subject'] !== '' ? htmlspecialchars($row['subject']) : 'No Subject').'</a> [ <a class=altlink href=sendmessage.php?receiver='.$CURUSER['id'].'&draft=1&edit_draft='.$row['id'].' title="edit this draft">e</a> ]</td>'.
'<td nowrap class='.$class.'>'.$row['added'].'</td>'.($_GET['check'] === 'yes' ? '<td class='.$class.'><input type=checkbox name="pm[]" value='.$row['id'].' /></td></tr>' : '<td class='.$class.'><input type=checkbox name="pm[]" value='.$row['id'].' /></td></tr>');
}
}
...
//=== Get the message
$res = mysql_query('SELECT * FROM messages WHERE id='.sqlesc($pm_id).' AND (receiver='.$CURUSER['id'].' OR (sender='.$CURUSER['id'].' AND (saved = \'yes\' || unread= \'yes\'))) LIMIT 1') or sqlerr(__FILE__,__LINE__);
...
//=== Mark message read
mysql_query("UPDATE messages SET unread='no' WHERE id=" . sqlesc($pm_id) . " AND receiver=".$CURUSER['id']." LIMIT 1") or sqlerr(__FILE__,__LINE__);
...
.($message['sender'] === $CURUSER['id'] ? 'To' : 'From').': '.($arr_user_stuff['username'] === 0 ? 'System' : ($message['sender'] === $CURUSER['id'] ? '<a class=altlink href=userdetails.php?id='.$message['receiver'].'>'.$arr_user_stuff['username'].'</a>' : '<a class=altlink href=userdetails.php?id='.$message['sender'].'>'.$arr_user_stuff['username'].'</a>')).
' &nbsp;&nbsp;</b>'.(($friend > 0) ? '&nbsp;[ <a class=altlink href=friends.php?action=delete&type=friend&targetid='.$id.'><font size="-4">remove from friends</font></a> ]' : ($id > 0 ? '&nbsp;[ <a class=altlink href=friends.php?action=add&type=friend&targetid='.$id.'><font size="-4">add to friends</font></a> ]' : '')).'&nbsp;'.
'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>sent on:</b> '.$message['added'].'&nbsp;&nbsp;'.(($message['sender'] === $CURUSER['id'] && $message['unread'] == 'yes') ? '<font color=red><b>[ Un-read ]</b></font>' : '').($message['urgent'] === 'yes' ? '&nbsp;&nbsp;&nbsp;<blink><font color=red><b>URGENT!</b></font></blink>' : '').'</td>'.
'</tr><tr><td width=83 rowspan=2 align=center valign=top class=clearalt7>'.$avatar.'</td><td colspan=2 class=clearalt6>'.format_comment($message['msg']).'</td></tr><tr><td class=clearalt7><form action=?action=move method=post>'.
'<input type=hidden name=id value='.$pm_id.'> <b>Move to:</b> '.get_all_boxes().' '.
' <input class=button type=submit value=Move></form></td><td align=right class=clearalt7>[ <a class=altlink href=?action=delete&id='.$pm_id.'>Delete</a> ] '.(($id < 1 || $message['sender'] === $CURUSER['id']) ? '' : ' [ <a class=altlink href=sendmessage.php?receiver='.$message['sender'].'&replyto='.$pm_id.'>Reply</a> ]')
...
echo'<br><table><tr><td class=colhead colspan=2>using draft: '.htmlspecialchars($message['subject']).'<form name=compose action=?action=send_draft method=post><input type=hidden name=id value='.$pm_id.'></td></tr>'.
'<tr><td class=clearalt6 valign=top align=right><b>To:</b></td><td class=clearalt6 valign=top><input type=text name=to value="Enter Username" size=83></td></tr>'.
'<tr><td class=clearalt6 valign=top align=right><b>Subject:</b></td><td class=clearalt6 valign=top><input type=text name=subject value="'.htmlspecialchars($message['subject']).'" size=83></td></tr>'.
'<tr><td class=clearalt6 valign=top align=right><b>Body:</b></td><td class=clearalt6 valign=top align=right>';
...
$res_username = mysql_query('SELECT id, class, acceptpms FROM users WHERE LOWER(username)=LOWER('.sqlesc(htmlspecialchars($_POST['to'])).') LIMIT 1');
...
$res2 = mysql_query('SELECT * FROM blocks WHERE userid='.sqlesc($to_username['id']).' AND blockid='.$CURUSER['id']) or sqlerr(__FILE__, __LINE__);
...
$res2 = mysql_query('SELECT * FROM friends WHERE userid='.sqlesc($to_username['id']).' AND friendid='.$CURUSER['id']) or sqlerr(__FILE__, __LINE__);
...
mysql_query("INSERT INTO messages (poster, sender, receiver, added, subject, msg, location, saved) VALUES(".$CURUSER['id'].", ".$CURUSER['id'].", ".sqlesc($to_username['id']).", ".sqlesc(get_date_time()).", ".sqlesc($subject).",".sqlesc($body).", ".sqlesc(PM_INBOX).", ".sqlesc($save).")") or sqlerr(__FILE__, __LINE__);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if ($action === 'move'){
@mysql_query('UPDATE messages SET location= '.sqlesc(0 + $_POST['box']).' WHERE id='.sqlesc(0 + $_POST['id']).' AND receiver='.$CURUSER['id']);
...
$res = mysql_query('SELECT receiver,sender,urgent,unread,saved,location FROM messages WHERE id='.$id) or sqlerr(__FILE__,__LINE__);
$message = mysql_fetch_assoc($res);
...
mysql_query('DELETE FROM messages WHERE id='.$id) or sqlerr(__FILE__,__LINE__);
elseif ($message['receiver'] == $CURUSER['id'] && $message['saved'] == 'yes')
mysql_query('UPDATE messages SET location=0, unread=\'no\' WHERE id='.$id) or sqlerr(__FILE__,__LINE__);
...
$res_forward = mysql_query('SELECT username FROM users WHERE id='.sqlesc($message['sender'])) or sqlerr(__FILE__,__LINE__);
...
echo'<h1>Fwd: '.htmlspecialchars($message['subject']).'</h1><form action=?action=forward_pm method=post><input type=hidden name=id value='.$pm_id.'>'.
'<table><tr><td class=colhead valign=top align=left colspan=2><h1>forward message <img src=pic/arrow_next.gif alt=":">Fwd: '.htmlspecialchars($message['subject']).'</h1></td></tr>'.
'<tr><td class=clearalt7 valign=top align=right><b>To:</b></td><td class=clearalt6 valign=top><input type=text name=to value="Enter Username" size=83></td>'.
'</tr><tr><td class=clearalt7 valign=top align=right><b>Orignal<BR>Receiver:</b></td><td class=clearalt6 valign=top><b>'.$forwarded_username.'</b></td>'.
'</tr><tr><td class=clearalt7 valign=top align=right><b>From:</b></td><td class=clearalt6 valign=top><b>'.$CURUSER['username'].'</b></td></tr><tr>'.
'<td class=clearalt7 valign=top align=right><b>Subject:</b></td><td class=clearalt6 valign=top><input type=text name=subject value="Fwd: '.htmlspecialchars($message['subject']).'" size=83></td></tr><tr>'.
'<td class=clearalt7 valign=top align=right><b>Message:</b></td><td class=clearalt6 valign=top><textarea name=msg cols=80 rows=8></textarea><br>'.
'-------- Original Message from '.$forwarded_username.': --------<BR>'.format_comment($message['msg']).'</td>'.
'</tr><tr><td colspan=2 align=center class=clearalt7>'.($CURUSER['class'] >= UC_MODERATOR? '<b><font color=red>Mark as URGENT!</font></b> <input type=checkbox name=urgent value=yes>&nbsp' : '').'
Save Message <input type=checkbox name=save value=1><input type=hidden name=first_from value="'.$forwarded_username.'">&nbsp;'.
'<input class=button type=submit value=Forward></td></tr></table></form>';
...
'<option value=no '.($CURUSER['show_pm_avatar'] === 'no' ? ' selected' : '').'>don\'t show avatars on PM list</option></select> please select if you would like to see avatars in your PM listings and click '.
'<input type=hidden name=edit_mail_boxes value=1><input class=button type=submit value=change></form><br><br></td></tr></table>';
...
//=== Try finding a user with specified name
if ($member){
$res_username = mysql_query('SELECT username,id FROM users WHERE LOWER(username)=LOWER('.sqlesc($member).') LIMIT 1') or sqlerr(__FILE__,__LINE__);
