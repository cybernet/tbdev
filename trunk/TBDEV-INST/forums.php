...
dbconn();
maxcoder();	
if ($FORUMS_ONLINE == 'no' AND $CURUSER['class'] < UC_ADMINISTRATOR)
...
function forum_stats()
{
	...
	$res = mysql_query("SELECT id, username, donor, warned, class, avatar FROM users WHERE forum_access >= ".sqlesc(get_date_time(gmtime() - 180))." ORDER BY forum_access DESC") or sqlerr(__FILE__, __LINE__);
	while ($arr = mysql_fetch_assoc($res))
	{
...
function show_forums($forid)
{
	...
	$forums_res = mysql_query("SELECT f.id, f.name, f.description, f.postcount, f.topiccount, f.minclassread, p.added, p.topicid, p.userid, p.id AS pid, u.username, t.subject, t.lastpost, r.lastpostread ".
							  "FROM forums AS f ".
			...
			<td align='left'>
				<table border=0 cellspacing=0 cellpadding=0>
					<tr>
						...
						{
							?>&nbsp;<font class='small'>[<a class='altlink' href='<?php echo $_SERVER['PHP_SELF']; ?>?action=editforum&forumid=<?php echo $forumid; ?>'>Edit</a>][<a class='altlink' href='<?php echo $_SERVER['PHP_SELF']; ?>?action=deleteforum&forumid=<?php echo $forumid; ?>'>Delete</a>]</font><?php
						}
...
if (!($CURUSER['post_count'] < $CURUSER['post_max']))
	...
	if ($newtopic)
	{
		...
		$postid = mysql_insert_id() or stderr("Error", "No post ID returned!");
	                ...
	                mysql_query("UPDATE users SET post_count = post_count + 1 WHERE id = ".sqlesc($CURUSER['id'])) or sqlerr(__FILE__, __LINE__);
                                }
	else
	{
...
else if ($action == "viewtopic") //-------- Action: View topic
{
	...
	//------ Get topic info
	$res = mysql_query("SELECT ".($use_poll_mod ? 't.pollid, ' : '')."t.locked, t.subject, t.sticky, t.userid AS t_userid, t.forumid, t.numratings, t.ratingsum, f.name AS forum_name, f.minclassread, f.minclasswrite, f.minclasscreate, (SELECT COUNT(id)FROM posts WHERE topicid = t.id) AS p_count ".
					   "FROM topics AS t ".
					   "LEFT JOIN forums AS f ON f.id = t.forumid ".
					   "WHERE t.id = ".sqlesc($topicid)) or sqlerr(__FILE__, __LINE__);
...
<?
	...
	while ($arr = mysql_fetch_assoc($res))
	{
		...
		echo ($pn == $pc ? '<a name=last></a>' : '');
	                ...
	                begin_table();
		?><tr><td width='737' colspan="2"><table class="main"><tr><td style="border:none;" width="100%"><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewtopic&topicid=<?=$topicid;?>&page=p<?=$postid;?>#<?=$postid;?>'>#<?=$postid;?></a> by <?=$by;?> <?=$title;?> <a href='usermood.php' onClick="return popitup('usermood.php')">
                                <span class='tool'><img border="0" src='<?php echo $pic_base_url;?>smilies/<?php echo safe($moodupic);?>' alt='' /><span class='tip'><?php echo safe($arr['uusername']);?> <?php echo safe($mooduname);?>!</span></span></a>  at <?php echo $added;  if (isset($newp)){echo ("$newp");} ?><?php
		?></td><td style="border:none;"><a href="#top"><img align="right" border="0" src='<?php echo $pic_base_url.$forum_pics['arrow_up']; ?>' alt='Top' /></a></td></tr></table></td></tr><?php
	...
	else
	{
		...
		if ($CURUSER['class'] < $arr["write"])
		{
			...
			if ($CURUSER['post_max'] == 0)
                                                ...
                                                print("<p><i>You have reached your posting limit. Please retry in 15 minutes.</i></p>\n");
...
else if ($action == "viewforum") //-------- Action: View forum
{
	...
	"SELECT t.id, t.userid, t.views, t.locked, t.sticky".($use_poll_mod ? ', t.pollid' : '').", t.subject, u1.username, r.lastpostread, p.id AS p_id, p.userid AS p_userid, p.added AS p_added, (SELECT COUNT(id) FROM posts WHERE topicid=t.id) AS p_count, u2.username AS u2_username ".
	"FROM topics AS t ".
				...
				<td align=left width="100%">
					<table border=0 cellspacing=0 cellpadding=0>
						<tr>
							...
							<td class=embedded align=left><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewtopic&topicid=<?php echo $topicid; ?>'><?php echo htmlspecialchars($topic_arr['subject']); ?></a>&nbsp;&nbsp;<?php echo ($sticky ? "<img src=pic/sticky.gif>  " : ""); ?><?php echo $topicpages; ?></td>
						</tr>
					</table>
				</td>
	...
	while ($ovf_arr = mysql_fetch_assoc($ovf_res))
	{
		...
		?><tr>
			<td align='left' class='colhead' width="100%">
				<a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=forumview&forid=<?php echo $ovfid; ?>'><b><font color='blue'><?php echo htmlspecialchars($ovfname); ?></font></b></a>
			</td>
			<td align='right' class='colhead'><font color='blue'><b>Topics</b></td>
			<td align='right' class='colhead'><font color='blue'><b>Posts</b></font></td>
			<td align='left' class='colhead'><font color='blue'><b>Last post</b></font></td>
		</tr><?php
