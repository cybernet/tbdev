...
if ($HTTP_SERVER_VARS["REQUEST_METHOD"] == "POST")
{
       ...
       $uploaded = sqlesc($HTTP_POST_VARS["uploaded"]);
       $downloaded = sqlesc($HTTP_POST_VARS["downloaded"]);

       sql_query("UPDATE users SET uploaded=$uploaded, downloaded=$downloaded WHERE username=$username") or sqlerr(__FILE__, __LINE__);
       write_log("$username had their ratio adjusted by $CURUSER[username] to $uploaded bytes uploaded and $downloaded bytes downloaded.");
