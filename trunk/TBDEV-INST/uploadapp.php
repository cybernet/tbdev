...
if ($_POST["form"] != "1") {
  ...
  elseif ($arr["status"] == "rejected")
	...
	print("<table width=750 border=1 cellspacing=0 cellpadding=10><tr><td>");
	...
	print("<tr><td class=rowhead>My username is</td><td><input name=userid type=hidden value=".$CURUSER['id'].">".$CURUSER['username']."</td></tr>");
    print("<tr><td class=rowhead>I have joined at</td><td>".$CURUSER['added']." (".get_elapsed_time(sql_timestamp_to_unix_timestamp($CURUSER["added"]))." ago)</td></tr>");
	print("<tr><td class=rowhead>I have a positive ratio</td><td>".($ratio >= 1 ? "Yes" : "No")."</td></tr>");
	print("<tr><td class=rowhead>I am connectable</td><td><input name=connectable type=hidden value=$connectable>$connectable</td></tr>");
	print("<tr><td class=rowhead>My upload speed is</td><td><input type=text name=speed size=19></td></tr>");
	print("<tr><td class=rowhead>What I have to offer</td><td><textarea name=offer cols=80 rows=1 wrap=VIRTUAL></textarea></td></tr>");
	print("<tr><td class=rowhead>Why I should be promoted</td><td><textarea name=reason cols=80 rows=2 wrap=VIRTUAL></textarea></td></tr>");
	print("<tr><td class=rowhead>I am uploader at other sites</td><td><input type=radio name=sites value=yes>Yes
	<input name=sites type=radio value=no checked>No</td></tr>");
	print("<tr><td class=rowhead>Those sites are</td><td><textarea name=sitenames cols=80 rows=1 wrap=VIRTUAL></textarea></td></tr>");
	print("<tr><td class=rowhead>I have scene access</td><td><input type=radio name=scene value=yes>Yes
	<input name=scene type=radio value=no checked>No</td></tr>");
	print("<tr><td colspan=2>");
	print("<br>&nbsp;&nbsp;I know how to create, upload and seed torrents");
	print("<br><input type=radio name=creating value=yes>Yes
	<input name=creating type=radio value=no checked>No");
	print("<br><br>&nbsp;&nbsp;I understand that I have to keep seeding my torrents until there are at least two other seeders");
	print("<br><input type=radio name=seeding value=yes>Yes
	<input name=seeding type=radio value=no checked>No");
	print("<br><br><input name=form type=hidden value=1>");
	print("</td></tr>");
	print("</table>");
