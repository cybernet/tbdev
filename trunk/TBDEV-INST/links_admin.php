...
print $row['mtime'];
echo "<form name='compose' method='post' action='links_admin.php'><input type='hidden' name='action' value='links_update' />".
"<input type='hidden' name='links_id' value='{$row['id']}' /><input type='text' value='{$row['heading']}' name='heading' style='width:380px;' /> ".
"<select name='cat'><option value=''>--Select One--</option>";

foreach($cats as $v) { print "<option value=$v[cid]>$v[rcat_name]</option>"; }
echo '</select><br />';
$body = safechar($row['body']);
textbbcode("compose","body",$body);
echo "<br /><script language='javascript' src='spellmessage.js'></script> ".
"<input type=button class=button value=\"Spell Check\" onclick=\"return openspell(1);\"> <input type='submit' name='submit' value='Edit This Entry' class='button'></form>";

//===added delete
echo "<form name='deleteform' method='post' action='links_admin.php'><input type='hidden' name='action' value='links_delete' />";
"<input type='hidden' name='id' value='{$row['id']}' /><br><input type='submit' name='submit' value='Delete This Entry' class='button'></form></div>";
}
}
Stdfoot();
exit();
