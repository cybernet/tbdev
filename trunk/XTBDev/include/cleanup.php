...
function docleanup() {
	global $torrent_dir, $signup_timeout, $max_dead_torrent_time, $autoclean_interval, $max_dead_user_time, $max_dead_topic_time, $ad_ratio, $ap_time;
  ...
  if (mysql_num_rows($res) > 0)
  {
	...
	$res = mysql_query("SELECT id FROM users WHERE class = 0 AND uploaded >= $ap_limit AND uploaded / downloaded >= $ap_ratio AND added < $maxdt") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) > 0)
	{
		$dt = sqlesc(get_date_time());
	...
	// demote power users
	$res = mysql_query("SELECT id FROM users WHERE class = 1 AND uploaded / downloaded < $ad_ratio") or sqlerr(__FILE__, __LINE__);
	if (mysql_num_rows($res) > 0)
	{
		$dt = sqlesc(get_date_time());
