...
function dltable($name, $arr, $torrent)
{
	...
	$s .= "<tr><td class=colhead>User/IP</td>" .
          ...
          "<td class=colhead align=right>Avg.&nbsp;Rate</td>".
...
$mod = get_user_class() >= UC_MODERATOR;
	foreach ($arr as $e) {
        ...
        $s .= "<td align=center>" . ($e[connectable] == "yes" ? $e["port"] : "---") . "</td>\n";
		$s .= "<td align=right>" . mksize($e["uploaded"]) . "</td>\n";
		$s .= "<td align=right>" . mksize($e["uploaded"] / $secs) . "/s</td>\n";
		$s .= "<td align=right>" . mksize($e["downloaded"]) . "</td>\n";
		$s .= "<td align=right>" . mksize($e["downloaded"] / $secs) . "/s</td>\n";
                if ($e["downloaded"])
                {
...
$s.="<tr><td class=colhead>Path</td><td class=colhead align=right>Size</td></tr>\n";
		...
		if (!$_GET["dllist"]) {
			...
			*/
			...
			$downloaders = array();
			$seeders = array();
			$subres = mysql_query("SELECT seeder, ip, port, uploaded, downloaded, to_go, UNIX_TIMESTAMP(started) AS st, connectable, agent, UNIX_TIMESTAMP(last_action) AS la, userid FROM peers WHERE torrent = $id") or sqlerr();
			while ($subrow = mysql_fetch_array($subres)) {
