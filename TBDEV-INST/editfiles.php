...
$_folder = htmlspecialchars(isset($_GET['folder'])?$_GET['folder']:$_POST['folder']);
...
$file = htmlspecialchars(isset($_GET['filename'])?$_GET['filename']:$_POST['filename']);
...
elseif ($action == 'edit')
{
	...
	$data = open_file($file);
	echo "<form action=\"".$_SERVER['PHP_SELF']."?action=save\" method=\"post\"><textarea name=\"data\" rows=\"50\" cols=\"150\">".htmlspecialchars($data)."</textarea><input type=\"hidden\" name=\"filename\" value=\"".$file."\">".(!empty($_folder)?"<input type=\"hidden\" name=\"folder\" value=\"".$_folder."\">":'')."<div align=\"center\"><input type=\"submit\" value=\"Save File\"></div></form>";
