...
$r = mysql_query("SELECT bjwins, bjlosses FROM users WHERE id=$CURUSER[id]");
$a = mysql_fetch_array($r);
$tot_wins = $a[bjwins];
...
if ($_POST["game"]) {
        ...
        if ($_POST["game"] == 'start'){
                ...
                if ($cardarr[points] == 1)
                        $cardarr[points] = 11;
                mysql_query("insert into blackjack (userid, points, cards, date) values($CURUSER[id], $cardarr[points], $cardid, $now)") or sqlerr(__FILE__, __LINE__);
                stdhead("Blackjack");
                ...
                print("<table class=message width=100% cellspacing=0 cellpadding=5 bgcolor=black>\n");
                print("<tr><td align=center><img src=pic/cards/".$cardarr["pic"]." width=71 height=96 border=0></td></tr>");
                ...
                mysql_query("update blackjack set points=$points, cards='$mysqlcards' where userid=$CURUSER[id]") or sqlerr(__FILE__, __LINE__);
                if ($points == 21) {
                        $waitres = mysql_query("select count(*) from blackjack where status='waiting' and userid!=$CURUSER[id]");
                        ...
                        if ($waitarr[0] > 0){
                                $r = mysql_query("select * from blackjack where status='waiting' and userid!=$CURUSER[id] order by date asc LIMIT 1");
                                $a = mysql_fetch_assoc($r);
                                if ($a["points"] != 21) {
                                        $winorlose = "you won ".mksize($mb);
                                        mysql_query("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$CURUSER[id]") or sqlerr(__FILE__, __LINE__);
                                        mysql_query("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$a[userid]") or sqlerr(__FILE__, __LINE__);
                                        ...
                                        mysql_query("INSERT INTO messages (sender, receiver, added, msg, poster, subject) VALUES(0, $a[userid], $dt, $msg, 0, $subject)") or sqlerr(__FILE__, __LINE__);
                                        } else {
                        ...
                        print("<tr><td align=center>Your opponent was ".get_user_name($a["userid"]).", they had $a[points] points, $winorlose.<br /><br /><center><b><a href=blackjack.php>Play again</a></b></center></td></tr>");
                                ...
                                print("<tr><td align=center>There are no other players, so you'll have to wait until someone plays against you.<br />You will receive a PM with the game results.<br /><br /><center><b><a href=blackjack.php>Back</a></b><br /></center></td></tr>");
                        }
                        print("</table>");
                        print("</td></tr></table><br>");
                        stdfoot();
                        ...
                        if ($waitarr[0] > 0) {
                                ...
                                if ($a["points"] > 21) {
                                        ...
                                        mysql_query("update users set uploaded = uploaded - $mb, bjlosses = bjlosses + 1 where id=$CURUSER[id]") or sqlerr(__FILE__, __LINE__);
                                        mysql_query("update users set uploaded = uploaded + $mb, bjwins = bjwins + 1 where id=$a[userid]") or sqlerr(__FILE__, __LINE__);
                                        mysql_query("delete from blackjack where userid=$CURUSER[id]");
                                        mysql_query("delete from blackjack where userid=$a[userid]");
                                        $dt = sqlesc(get_date_time());
                        ...
                        print("</form>");
                        print("<form name=blackjack method=post action=$phpself>");
                        print("<tr><td align=center><input type=hidden name=game value=stop><input type=submit value=' Stay '></td></tr>");
                        print("</form>");
                        print("</table>");
                        print("</td></tr></table><br>");
                        stdfoot();
        ...
        print("<table class=message width=100% cellspacing=0 cellpadding=10 bgcolor=black>\n");
        print("<tr><td align=center><img src=pic/cards/tp.bmp width=71 height=96 border=0> <img src=pic/cards/vp.bmp width=71 height=96 border=0> </td></tr>\n");
        print("<tr><td align=left>You must collect 21 points without going over.<br><br>\n");
        print("<b>NOTE:</b> By playing blackjack, you are betting 100 MB of upload credit!</td></tr>\n");
        ...
        print("<table cellspacing=0 cellpadding=3 width=400>\n");
        print("<tr><td colspan=2 cellspacing=0 cellpadding=5 align=center>\n");
        print("<h1><a href=bjstats.php?type=3><center>Personal Statistics</center></a></h1>\n");
        print("<tr><td align=left><b>Wins</b></td><td align=center><b>$tot_wins</b></td></tr>\n");
        print("<tr><td align=left><b>Losses</b></td><td align=center><b>$tot_losses</b></td></tr>\n");
        print("<tr><td align=left><b>Games Played</b></td><td align=center><b>$tot_games</b></td></tr>\n");
        print("<tr><td align=left><b>Win Percentage</b></td><td align=center><b>$win_perc</b></td></tr>\n");
