...
parked();
$action = safechar(trim($_GET["action"]));
...
mysql_free_result($res);
  ...
  function insert_compose_frame($id, $newtopic = true, $quote = false, $attachment = false)
  {
    ...
    else
    {
      ...
      print("<p align=center><h3>Reply to topic: <a href=?action=viewtopic&topicid=$id>".safechar($subject)."</a></h3></p>");
    }
    ...
    if (!$newtopic)
    {
      ...
      while ($post = mysql_fetch_assoc($postres))
      {
        ...
        $user = mysql_fetch_assoc($userres);
      	...
      	$avatar = ($CURUSER["avatars"] == "yes" ? safechar($user["avatar"]) : "");
//	    $avatar = $user["avatar"];
...
<?
		...
		//---------------------------------	
		$highlight = safechar(trim($_GET["highlight"]));
		//---------------------------------
		//---- Search Highlight v0.1 by xam
		//---------------------------------
...
print("<tr><td width=1% class=embedded$c>&nbsp;&nbsp;<nobr>" . safechar($a[1]) . " </nobr></td><td width=99% class=embedded$c>" .
...
while ($a = $o[$i])
{
print("<input type=radio name=choice value=$i>".safechar($a)."<br>\n");
++$i;
}
print("<br>");
...
stderr("Error","Poll doesn't exist. Contact SysOp");
    ...
    print("<a name=top><h2><a href=?action=viewforum&forumid=$forumid>$forum</a> &gt; ".safechar($subject)."</h2>\n");
...
$rem = sql_query("SELECT COUNT(*) FROM posts WHERE userid=" . $posterid) or sqlerr(__FILE__, __LINE__);
 ...
 $forumposts = $arr25[0];
      ...
      {
//		if ($arr2["enabled"] == "yes")
	        $avatar = ($CURUSER["avatars"] == "yes" ? safechar($arr2["avatar"]) : "");
//	    else
//			$avatar = "pic/disabled_avatar.gif";
 ...
 $UC = array("SYSOP" => "pic/class/coder.gif",
 "ADMINISTRATOR" => "pic/class/admin.gif",
 "MODERATOR" => "pic/class/mod.gif",
 "UPLOADER" => "pic/class/uploader.gif",
 "VIP" => "pic/class/vip.gif",
 "POWER_USER" => "pic/power.gif",
 "USER" => "pic/class/user.gif");
        $uclass = $UC[get_user_class_name($arr2["class"])];
        $res6 = sql_query("SELECT id FROM posts WHERE userid=$posterid");
$numberofposts = mysql_num_rows($res6);
if($numberofposts >=1 && $numberofposts <= 24)
$forumtitle = "Forum-Newbie";
elseif ($numberofposts >=25 && $numberofposts <= 150)
$forumtitle = "Forum-Lover";
elseif ($numberofposts >=151)
$forumtitle = "Forum-King";
...
//..rp..
      ...
      $body = format_comment(URLtitles($arr["body"]));
      ...
      if ($resat['filename'] AND $resat['id']){
		   if ($resat['postid'] == $postid) {			  
				...
				$body .= "<div style=\"padding:6px\"><fieldset class=\"fieldset\">
					...
					<td class=\"none\"><a href=\"attachment.php?attachmentid=$resat[id]\">".safechar($resat[filename])."</a> (".mksize($resat[size]).", $resat[downloads] views)</td>
     ...
     print("<tr valign=top><td width=150 align=left style='padding: 0px'><br>"."&nbsp; " .
       ($avatar ? "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<img width=100 src=\"$avatar\">": ""). "<br><br><br>"."$showgroup Rank: $forumtitle<br>$stats<br></td><td class=comment>$body</td></tr>\n");
	print("<tr><td> $onoffpic <a href=\"sendmessage.php?receiver=".safechar($posterid)."\"> <img src=\"pic/pm.gif\" border=\"0\" alt=\"Nachricht an ".safechar($postername)."\"></a> <a href=report.php?type=Post&id=$postid&id_2=$topicid><img src=\"pic/report.gif\" border=\"0\" alt=\"Report this post\"></a></td>");
...
print("<input type=hidden name=topicid value=".safechar($topicid).">\n");
print("<input type=hidden name=returnto value=\"".safechar($HTTP_SERVER_VARS["REQUEST_URI"])."\">\n");
...
//..rp..
    ...
    //------ Mod options
	  ...
	  if (get_user_class() >= UC_MODERATOR)
	  {
	    ...
	    print("<input type=hidden name=topicid value=$topicid>\n");
	    print("<input type=hidden name=returnto value=$_SERVER[REQUEST_URI]>\n");
	    print("<tr><td class=embedded align=right>Rename topic:</td><td class=embedded><input type=text name=subject size=60 maxlength=$maxsubjectlength value=\"" . safechar($subject) . "\">\n");
	    print("<input type=submit value='Ok' class=gobutton></td></tr>");
	    print("</form>\n");
...
if ($action == 'deletetopic' && get_user_class() >= UC_MODERATOR) //-------- Action: Delete topic
{
 ...
 //=== saving history of post edits [snuggs added]
if ($arr['editedby'] == 0)
{
    $res_first = sql_query("SELECT username FROM users WHERE id=".$arr['userid']);
    $arr_edited = mysql_fetch_assoc($res_first);
    
    $post_history = '[b]First Post by [url='.$DEFAULTBASEURL.'/userdetails.php?id='.$arr['userid'].'] '.$arr_edited['username'].'[/url] made at: '.$arr['added'].' GMT[/b][hr]'.$arr['body'].'[hr][b]Post edited by [url='.$DEFAULTBASEURL.'/userdetails.php?id='.$CURUSER['id'].']'.$CURUSER['username'].'[/url] at: '.get_date_time().' GMT[/b][hr]'.$_POST['body'];
}
else
    $post_history = $arr['post_history'].'[hr][b]Post edited by [url='.$DEFAULTBASEURL.'/userdetails.php?id='.$CURUSER['id'].'] '.$CURUSER['username'].'[/url] at: '.get_date_time().' GMT[/b][hr]'.$_POST['body'];

sql_query("UPDATE posts SET body=$body, post_history=".sqlesc($post_history).(!$_POST['lasteditedby'] ? ", editedat=$editedat, editedby=".$CURUSER['id'] : '')." WHERE id=$postid") or sqlerr(__FILE__, __LINE__);
       
          //sql_query("UPDATE posts SET body=$body, post_history=$post_history, editedat=$editedat, editedby=$CURUSER[id] WHERE id=$postid") or sqlerr(__FILE__, __LINE__);
   ...
   print("<input type=hidden name=returnto value=\"" . safechar($HTTP_SERVER_VARS["HTTP_REFERER"]) . "\">\n");

   print("<p align=center><table class=main border=1 cellspacing=0 cellpadding=5 width=737>\n");

   print("<tr><td class=rowhead>Body</td><td align=left style='padding: 0px'>");
       
   textbbcode("edit","body",safechar(unesc($arr["body"])));
...
<td align="left">
<a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewtopic&topicid=<?php echo (int)$arr['id']; ?>&page=last#last'><?php echo safechar($arr['subject']); ?></a><br />in&nbsp;<font class="small"><a href='<?php echo $_SERVER['PHP_SELF']; ?>?action=viewforum&forumid=<?php echo (int)$arr['forumid']; ?>'><?php echo safechar($arr['name']); ?></a></font>
</td>
<td align="center">
...
stderr("Sorry...", "There are no unread posts.<br /><br />Click <a href=".$_SERVER['PHP_SELF']."?action=getdaily>here</a> to get today's posts (last 24h).");
  ...
  if ($action == "getdaily") {
	...
	{		
		print("<tr><td><a href=\"forums.php?action=viewtopic&topicid={$getdaily["tid"]}&page=p{$getdaily["pid"]}#{$getdaily["pid"]}\"><b>".safechar($getdaily["subject"])."</b></a><br />in <a href=\"forums.php?action=viewforum&forumid={$getdaily["forumid"]}\">{$getdaily["name"]}</a></td>".
...
if ($action == "search")
{
	...
	unset($error);
	$error= false;
	$keywords = safechar(trim($_GET["keywords"]));
	if ($keywords != "")
	{
		$perpage = 5;
		...
		else
		{
			...
			for ($i = 0; $i < $num; ++$i)
			{
				...
				//---- Search Highlight v0.1 by xam
				//---------------------------------	
				print("<tr><td>$post[id]</td><td align=left><a href=?action=viewtopic&highlight=$keywords&topicid=$post[topicid]&page=p$post[id]#$post[id]><b>" . safechar($topic["subject"]) . "</b></a></td><td align=left><a href=?action=viewforum&forumid=$topic[forumid]><b>" . safechar($forum["name"]) . "</b></a><td align=left><b><a href=userdetails.php?id=$post[userid]>$user[username]</a></b><br>at $post[added]</tr>\n");
				//---------------------------------
				//---- Search Highlight v0.1 by xam
				//---------------------------------
			}
...
// - Bleaches Edits
  ...
  while ($forums_arr = mysql_fetch_assoc($forums_res))
  {
    ...
    $forumname = safechar($forums_arr["name"]);

    $forumdescription = safechar($forums_arr["description"]);
    ...
    if (mysql_num_rows($post_res) == 1)
    {
      ...
      $lastposter = safechar($user_arr['username']);
      ...
      $lasttopic = safechar($topic_arr['subject']);
