...
hacker_dork("Mass-Email- Nosey Cunt !");
$class = 0 + $_POST["class"];
$or = $_POST["or"];

if ($HTTP_SERVER_VARS["REQUEST_METHOD"] == "POST")
{
$res = sql_query("SELECT id, username, email FROM users WHERE class $or $class") or sqlerr(__FILE__, __LINE__);

$from_email = "noreply@yoursite"; //site email
...
$to = $arr["email"];
$message = "Message received from Yoursite on " . gmdate("Y-m-d H:i:s") . " GMT.\n" .
"---------------------------------------------------------------------\n\n" .
$message1 . "\n\n" .
"---------------------------------------------------------------------\n$SITENAME\n";
