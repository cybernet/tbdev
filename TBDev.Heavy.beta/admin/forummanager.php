...
case 'delete' : 
	...
	if(mysql_query('DELETE f.*,t.*,p.*,r.* FROM forums AS f LEFT JOIN topics AS t ON t.forumid = f.id LEFT JOIN posts AS p ON p.topicid = t.id  LEFT JOIN readposts AS r ON r.topicid = t.id WHERE f.id ='.$id)) {
...
$r1 = mysql_query('select f.name as f_name, f.id as fid, f.description,f.minclassread,f.minclasswrite, f.minclasscreate, o.name as o_name,o.id as oid FROM forums as f LEFT JOIN forum_parents as o ON f.forid = o.id ORDER BY f.sort') or  sqlerr(__FILE__,__LINE__);
...
$htmlout .= stdmsg('Err','There are no topics, maybe you should add some');
	...
	if($do =='edit' && $id) {
		$r3 = mysql_query('select f.name as f_name , f.id as fid , f.description , f.minclassread , f.minclasswrite , f.minclasscreate, f.forid, f.sort FROM forums as f WHERE f.id ='.$id) or sqlerr(__FILE__,__LINE__);
	...
	$htmlout .= "<tr><td align='right' valign='top'>Overforum</td><td align='left'><select name='overforum'><option value='0'>Select an option</option>";
	$r2 = mysql_query('SELECT id,name FROM forum_parents ORDER BY name') or sqlerr(__FILE__,__LINE__);
